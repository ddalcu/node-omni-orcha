    ...
    token_callback_t on_token)
{
  ...
  // Reasoning: disabled when thinkingBudget == 0, enabled otherwise
  bool enable_thinking = (thinking_budget != 0);
  inputs.reasoning_format = enable_thinking ? COMMON_REASONING_FORMAT_DEEPSEEK : COMMON_REASONING_FORMAT_NONE;
  ...
  // Preserved tokens (special tokens to render as text)
  ...
  // Reasoning budget: when thinking_budget > 0, enforce a token limit on the thinking block.
  // When budget exhausts, force a transition message + </think> to guide the model into content.
  if (enable_thinking && thinking_budget > 0 && !chat_params.thinking_end_tag.empty()) {
    sparams.reasoning_budget_tokens = thinking_budget;
    sparams.reasoning_budget_start = common_tokenize(vocab, chat_params.thinking_start_tag, false, false);
    sparams.reasoning_budget_end   = common_tokenize(vocab, chat_params.thinking_end_tag, false, false);

    // Build forced sequence: transition message + end tag
    // The message helps the model understand it should stop reasoning and produce content
    std::string budget_msg = "\n...I'll respond now.\n";
    auto msg_tokens = common_tokenize(vocab, budget_msg, false, false);
    auto end_tokens = sparams.reasoning_budget_end;
    msg_tokens.insert(msg_tokens.end(), end_tokens.begin(), end_tokens.end());
    sparams.reasoning_budget_forced = msg_tokens;
  }
  ...
  // Strip generation prompt from output
  ...
  // Parse output — extract content, reasoning, and tool calls
  if (!output.empty()) {
    try {
      common_chat_parser_params parser_params(chat_params);
      // Always use DEEPSEEK format for parsing — strips <think> tags from content
    ...
    // Strip residual <think>...</think> tags from content (happens when thinking is disabled
    // but model still emits empty think blocks)
    auto& c = result.content;
    while (true) {
      auto start = c.find("<think>");
      if (start == std::string::npos) break;
      auto end = c.find("</think>", start);
      if (end == std::string::npos) {
        c.erase(start); // strip from <think> to end
        break;
      }
...
void LlmContext::Register(Napi::Env env, Napi::Object& exports) {
  Napi::Function func = DefineClass(env, "LlmContext", {
    InstanceMethod("complete", &LlmContext::Complete),
    InstanceMethod("stream", &LlmContext::Stream),
    InstanceMethod("embed", &LlmContext::Embed),
  ...
  exports.Set("createLlmContext", Napi::Function::New(env, CreateLlmContext));
}

...
Unified native Node.js inference engine — single `omni.node` for LLM, TTS (Qwen3), and Image/Video generation.

## Architecture

- **Single `omni.node`** — all engines share one ggml build, one binary, no symbol conflicts
- **`engine/`** — llama.cpp fork with qwen3-tts, mtmd (multimodal/vision), and stable-diffusion sources compiled against shared ggml
...
engine/        llama.cpp fork (ggml + LLM core)
  ...
  vendor/      Third-party (cpp-httplib, stb, nlohmann)
...
scripts/       Build, download, and test helpers
  download-test-models.sh     Model downloader (LLM, TTS, Image, Video, Vision)
packages/      Platform-specific npm packages (darwin-arm64, linux-x64, etc.)
server.ts      Web UI — chat, TTS, image/video generation
...
Web UI with tabs for Chat, TTS, Image, and Video. Models load on-demand when you click a tab.
...
- **Custom binding over node-llama-cpp** — node-llama-cpp only covers LLM. We need LLM + TTS + Image/Video sharing one ggml build to avoid symbol conflicts and GPU backend contention. Separate binaries for each engine would mean duplicate ggml symbols, competing VRAM allocation, and more total complexity.
...
CI/CD (.github/workflows/build-and-publish.yml) builds all platforms.

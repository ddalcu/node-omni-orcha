...
// Converts 16-bit signed integer PCM to float32 [-1, 1]
static std::vector<float> pcm16_to_f32(const uint8_t* data, size_t byte_len) {
  size_t n_samples = byte_len / 2; // 16-bit = 2 bytes per sample
...
protected:
  void Execute() override {
    ...
    wparams.no_timestamps = false; // we want timestamps for segments
    ...
    for (int i = 0; i < n_segments; i++) {
      ...
      int64_t t0 = whisper_full_get_segment_t0(ctx_, i); // in 10ms units
...
protected:
  void Execute() override {
    ...
    // Only process enough audio for language detection (first 30s max)
    size_t max_samples = 16000 * 30; // 30 seconds at 16kHz
...
Napi::Value CreateSttContext(const Napi::CallbackInfo& info) {
  auto env = info.Env();
...
Napi::Object InitSttModule(Napi::Env env, Napi::Object exports) {
  Napi::Function func = SttContext::DefineClass(env, "SttContext", {
    SttContext::InstanceMethod("transcribe", &SttContext::Transcribe),
    SttContext::InstanceMethod("detectLanguage", &SttContext::DetectLanguage),
    SttContext::InstanceMethod("unload", &SttContext::Unload),
  });
  ...
  *constructor = Napi::Persistent(func);
  ...
  exports.Set("createContext", Napi::Function::New(env, CreateSttContext));
  return exports;
}

NODE_API_MODULE(stt, InitSttModule)
